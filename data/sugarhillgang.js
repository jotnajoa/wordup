const fs = require('fs');
let returnArray = [];
let prelyrics = `I said a hip-hop, the hippie, the hippie
...
let newlyrics = prelyrics.replace(/\n/g, ' ')
    // remove newline character

let cleanupbracket = newlyrics.replace(/ *\[[^\]]*]/g, ' ')
    // remove characters within []

let nobracket = cleanupbracket.replace(/ *\([^)]*\) */g, ' ');
// remove characters within ()

let nocurlybracket = nobracket.replace(/\s?\{[^}]+\}/g, ' ')
    // remove characer within {}

let noNonEnglish = nocurlybracket.replace(/[^\w'-]|_/g, ' ');
// remove nonAlphanumeric character but ' and -

let noNumber = noNonEnglish.replace(/[0-9]/g, '');

let finallyrics = noNumber.split(' ');

let date = 1980;
let lyrics = [...new Set(finallyrics)];
let songId = 453
let wordcount = lyrics.length

let obj = { lyrics, date, songId, wordcount }

returnArray.push(obj)

fs.writeFileSync('./sugarhill.json', JSON.stringify(returnArray))